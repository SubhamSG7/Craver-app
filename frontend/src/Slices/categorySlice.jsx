...
const categorySlice = createSlice({
    ...
    reducers: {
        ...
        setList: (state, action) => {
            const uniqueCategories = [...new Set(state.categories.map(val => val.category))];
        ...
        filterMenu: (state, action) => {
            ...
            let filteredData = state.categories.filter((val) => {
                const matchesSearch = search? val.name.toLowerCase().includes(search.toLowerCase()) : true;
                const matchesCategory = category? val.category === category : true;
                const matchesCuisine = cuisine? val.cuisine === cuisine : true;
                return matchesSearch && matchesCategory && matchesCuisine;
            })
          filteredData.length>0?state.filteredData=filteredData:state.filteredData="Not Found"
        }
    },
